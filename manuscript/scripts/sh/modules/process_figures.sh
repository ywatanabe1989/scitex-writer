#!/bin/bash
# -*- coding: utf-8 -*-
# Timestamp: "2025-05-06 12:47:36 (ywatanabe)"
...
init() {
    ...
    mkdir -p "$FIGURE_SRC_DIR" "$FIGURE_COMPILED_DIR" "$FIGURE_JPG_DIR" "$FIGURE_HIDDEN_DIR" "$FIGURE_COMPILED_DIR/debug"
    ...
    if [[ "$no_figs" == false ]]; then
        ...
        if [ -f "./scripts/py/optimize_figure.py" ] && command -v python3 >/dev/null 2>&1; then
            find "$FIGURE_SRC_DIR" -name "Figure_ID_*.tif" | parallel -j+0 --eta '
                python3 ./scripts/py/optimize_figure.py --input {} --output "'"$FIGURE_JPG_DIR"'/$(basename {} .tif).jpg" --dpi 300 --quality 95
            '

            find "$FIGURE_SRC_DIR" -name "Figure_ID_*.png" | parallel -j+0 --eta '
                python3 ./scripts/py/optimize_figure.py --input {} --output "'"$FIGURE_JPG_DIR"'/$(basename {} .png).jpg" --dpi 300 --quality 95
            '

            if command -v inkscape >/dev/null 2>&1; then
                find "$FIGURE_SRC_DIR" -name "Figure_ID_*.svg" | parallel -j+0 --eta '
                    inkscape -z -e "'"$FIGURE_JPG_DIR"'/$(basename {} .svg)_temp.png" -w 1200 -h 1200 {}
                    python3 ./scripts/py/optimize_figure.py --input "'"$FIGURE_JPG_DIR"'/$(basename {} .svg)_temp.png" --output "'"$FIGURE_JPG_DIR"'/$(basename {} .svg).jpg" --dpi 300 --quality 95
            ...
            find "$FIGURE_SRC_DIR" -name "Figure_ID_*.jpg" | parallel -j+0 --eta '
                if [ ! -f "'"$FIGURE_JPG_DIR"'/$(basename {})" ]; then
                    python3 ./scripts/py/optimize_figure.py --input {} --output "'"$FIGURE_JPG_DIR"'/$(basename {})" --dpi 300 --quality 95
                fi
            '
        else
            find "$FIGURE_SRC_DIR" -name "Figure_ID_*.tif" | parallel -j+0 --eta '
