...
pptx2tif() {
    ...
    if [[ "$p2t" == true ]]; then
        ...
        if [ -f "./scripts/shell/modules/pptx2tif_all.sh" ]; then
            # Run the conversion script
            run_command_verbose "./scripts/shell/modules/pptx2tif_all.sh" "PowerPoint to TIF conversion"
        else
        ...
        if [ -f "./scripts/python/crop_tif.py" ] && command -v python3 >/dev/null 2>&1; then
            ...
            if command -v parallel >/dev/null 2>&1; then
                ...
                if ! find "$FIGURE_SRC_DIR" -name "Figure_ID_*.tif" | 
                     parallel -j+0 --eta 'python3 ./scripts/python/crop_tif.py -l {}'; then
                ...
                for tif_file in "${tif_files[@]}"; do
                    ...
                    if ! python3 ./scripts/python/crop_tif.py -l "$tif_file"; then
                ...
                if command -v parallel >/dev/null 2>&1; then
                    ...
                    if [ "$img_type" = "svg" ] && command -v inkscape >/dev/null 2>&1; then
                        ...
                        find "$FIGURE_SRC_DIR" -name "Figure_ID_*.$img_type" | parallel -j+0 --eta '
                            ...
                            python3 ./scripts/python/optimize_figure.py --input "'"$FIGURE_JPG_DIR"'/$(basename {} .'"$img_type"')_temp.png" --output "'"$FIGURE_JPG_DIR"'/$(basename {} .'"$img_type"').jpg" --dpi 300 --quality 95 >> '"$log_file"'
                        ...
                        find "$FIGURE_SRC_DIR" -name "Figure_ID_*.$img_type" | parallel -j+0 --eta '
                            if [ ! -f "'"$FIGURE_JPG_DIR"'/$(basename {})" ]; then
                                ...
                                python3 ./scripts/python/optimize_figure.py --input {} --output "'"$FIGURE_JPG_DIR"'/$(basename {})" --dpi 300 --quality 95 >> '"$log_file"'
                            fi
                        ...
                        find "$FIGURE_SRC_DIR" -name "Figure_ID_*.$img_type" | parallel -j+0 --eta '
                            ...
                            python3 ./scripts/python/optimize_figure.py --input {} --output "'"$FIGURE_JPG_DIR"'/$(basename {} .'"$img_type"').jpg" --dpi 300 --quality 95 >> '"$log_file"'
                    ...
                    for img_file in "${img_files[@]}"; do
                        ...
                        if [ "$img_type" = "svg" ] && command -v inkscape >/dev/null 2>&1; then
                            ...
                            if inkscape -z -e "$FIGURE_JPG_DIR/${basename_no_ext}_temp.png" -w 1200 -h 1200 "$img_file"; then
                                python3 ./scripts/python/optimize_figure.py --input "$FIGURE_JPG_DIR/${basename_no_ext}_temp.png" --output "$FIGURE_JPG_DIR/${basename_no_ext}.jpg" --dpi 300 --quality 95 >> "$log_file"
                            ...
                            if [ ! -f "$FIGURE_JPG_DIR/$(basename "$img_file")" ]; then
                                ...
                                python3 ./scripts/python/optimize_figure.py --input "$img_file" --output "$FIGURE_JPG_DIR/$(basename "$img_file")" --dpi 300 --quality 95 >> "$log_file" || 
                                    log_error "Error optimizing JPG: $img_file" "$log_file"
                            fi
                        else
                            # TIF and PNG with Python
                            log_debug "Converting $img_type $img_file to JPG" "$log_file"
                            python3 ./scripts/python/optimize_figure.py --input "$img_file" --output "$FIGURE_JPG_DIR/${basename_no_ext}.jpg" --dpi 300 --quality 95 >> "$log_file" || 
