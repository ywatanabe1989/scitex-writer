...
function insert_citations() {
  ...
  echo
  if [[ $REPLY =~ ^[Yy]$ ]]; then
    echo $files_to_insert_citations | xargs -n 1 -P 5 ./.env/bin/python ./scripts/python/insert_citations.py -lb "./bibliography.bib" -lt
  fi
}
...
# ./scripts/shell/insert_citations.sh ./src/introduction.tex ./bibliography.bib
