...
def convert_pptx_to_tif_libreoffice(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            tif_files = [os.path.join(temp_dir, f) for f in os.listdir(temp_dir) 
            ...
            for i, tif_file in enumerate(tif_files):
                ...
                with open(tif_file, 'rb') as src_file, open(output_path, 'wb') as dst_file:
...
def convert_pptx_to_tif_python(
    ...
    for i, slide in enumerate(prs.slides):
        ...
        print(f"Warning: Python-only conversion is limited. Slide {i+1} would be saved to {output_path}")
        print("For better results, please install LibreOffice or use the LibreOffice conversion method")
...
def convert_pptx_to_tif(
    ...
    if crop_whitespace and output_files:
        ...
        try:
            ...
            print("Warning: crop_tif module not available. Skipping whitespace cropping.")
...
def batch_convert_pptx_to_tif(
    ...
    if recursive:
        ...
        pptx_files = [os.path.join(directory, f) for f in os.listdir(directory) 
...
def main():
    ...
    file_parser.add_argument("-i", "--input", required=True, help="Input PowerPoint file path")
    file_parser.add_argument("-o", "--output-dir", help="Output directory for TIF files")
    
    # Batch mode
    batch_parser = subparsers.add_parser("batch", help="Process multiple files")
    batch_parser.add_argument("-d", "--directory", required=True, help="Directory containing PowerPoint files")
    batch_parser.add_argument("-o", "--output-dir", help="Output directory for TIF files")
    ...
    for subparser in [file_parser, batch_parser]:
        subparser.add_argument("--method", choices=["auto", "libreoffice", "python"], default="auto", 
                             help="Conversion method to use")
        subparser.add_argument("--resolution", type=int, default=300, help="Output image resolution (DPI)")
        subparser.add_argument("--no-crop", action="store_true", help="Disable automatic cropping of whitespace")
        subparser.add_argument("--margin", type=int, default=30, help="Margin size around the content area when cropping")
