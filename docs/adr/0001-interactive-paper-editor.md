...
- **Mark on a figure, intent "change this"** → a **figure-change-request to figrecipe** (its lane). Payload **`figure-requests/1`**: `{claim_id = sanitized save-path, recipe_ref, panel{row,col}|label, region{x0,y0,x1,y1 normalized 0..1} + optional stroke_path, request_text, element_ref|null, source}`; minimum = `claim_id + panel + region + request_text`. figrecipe's **hitmap resolves the element** (`fr.save(save_hitmap=True)`); writer does no resolution. **Dispatch:** a2a nudge (promptness) + durable `.scitex/writer/figure_requests.json` (SSoT, merge-by-source, status `open→addressed`). **Reuse figrecipe surfaces** — figure pane = `figrecipe._django` (preview) + `figrecipe._hitmap` (targeting) + `figrecipe._editor`/`_call_overrides` (apply). Writer builds no figure rendering.
...
- **Coordinate map:** join by `claim_id`; clew adds a per-claim `pdf_anchor` slug (`[A-Za-z0-9_-]`) to the 1.6 feed, byte-identical to the LaTeX `\hypertarget{clew-<id>}` (card `writer-clew-macros-hypertarget-anchors`) — zero mapping table.
...
- **Gate policy:** today "no registered sources → PASS" (defensive). Add an explicit note + a **strict-mode knob** so this is a compose-time convenience, not a publish loophole.
