#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Test file for: manuscript_findings.py (the dynamic-paper findings feed)
...
from manuscript_findings import (  # noqa: E402
    build_feed,
    collect_findings,
...
class TestFindingsFromLog:
    def test_undefined_reference_becomes_reference_finding(self):
        # Arrange
        log = "LaTeX Warning: Reference `fig:1' on page 2 undefined on input line 9."
        # Act
        found = findings_from_log(log)
        # Assert
...
class TestFindingsFromClaims:
    ...
    def test_finding_carries_claim_id_and_location(self):
        # Arrange
...
class TestBuildFeed:
    _FINDINGS = [
        {"id": "a", "kind": "claim", "severity": "advice", "message": "m",
         "location": {}, "claim_id": None, "source": "clew"},
        {"id": "b", "kind": "reference", "severity": "warning", "message": "m",
         "location": {}, "claim_id": None, "source": "log"},
    ]
    ...
    def test_feed_declares_schema_version(self):
        ...
        # Act
        feed = build_feed(findings)
        # Assert
        assert feed["schema"] == "manuscript-findings/1"
    ...
    def test_findings_sorted_most_severe_first(self):
        ...
        feed = build_feed(findings)
        # Assert
        assert feed["findings"][0]["severity"] == "warning"
...
class TestCollectAndWrite:
    def test_collect_reads_log_and_claims_from_project(self, tmp_path):
        ...
        # Act
        found = collect_findings(str(tmp_path))
        # Assert
        assert len(found) == 2
    ...
    def test_write_feed_creates_json_file(self, tmp_path):
        ...
        # Assert
        assert json.loads(Path(out).read_text())["schema"] == "manuscript-findings/1"

    def test_absent_artifacts_yield_empty_feed_not_error(self, tmp_path):
        # Arrange
        empty_project = tmp_path
        # Act
        found = collect_findings(str(empty_project))
        # Assert
        assert found == []
