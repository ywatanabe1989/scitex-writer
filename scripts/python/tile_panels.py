#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Timestamp: "2025-09-27 19:30:00 (ywatanabe)"
# File: tile_panels.py
...
IO:
  - input-files:
    - Figure_ID_XX_name_A.jpg, Figure_ID_XX_name_B.jpg, etc.
  - output-files:
    - Figure_ID_XX_name.jpg (tiled composite)
"""
...
def detect_panels(figure_base, search_dir):
    ...
    # Extract figure ID from base (e.g., "01" from "Figure_ID_01_demographic_data")
    figure_id = figure_base.split('_')[2]  # Figure_ID_XX_name -> XX
    
    # Look for panel files: Figure_ID_01A_name.jpg, Figure_ID_01a_name.jpg, etc.
    for panel_file in search_path.glob(f"Figure_ID_{figure_id}[A-Za-z]_*.jpg"):
        # Extract panel letter from filename (e.g., "A" from "Figure_ID_01A_name.jpg")
        filename = panel_file.stem
        # Find the position where the figure ID ends and panel letter begins
        id_part = f"Figure_ID_{figure_id}"
        panel_letter = filename[len(id_part)].upper()  # Get the letter and convert to uppercase
    ...
    for offset_x in range(-outline_width, outline_width + 1):
        for offset_y in range(-outline_width, outline_width + 1):
            if offset_x != 0 or offset_y != 0:  # Don't draw on center
                draw.text((text_x + offset_x, text_y + offset_y), label, fill="white", font=font)
...
def add_panel_label_to_composite(composite_image, label, panel_x, panel_y, font_size=200):
...
def tile_images(panels, output_path, spacing=20, dpi=300):
    ...
    for label in images:
        ...
        images[label] = images[label].resize((target_width, target_height), Image.Resampling.LANCZOS)
...
def main():
    parser = argparse.ArgumentParser(description='Tile figure panels with automatic layout')
    parser.add_argument('--figure-base', required=True, help='Base figure name (e.g., Figure_ID_01_workflow)')
    parser.add_argument('--search-dir', required=True, help='Directory to search for panel files')
    parser.add_argument('--output', required=True, help='Output tiled image path')
    parser.add_argument('--spacing', type=int, default=20, help='Spacing between panels in pixels')
