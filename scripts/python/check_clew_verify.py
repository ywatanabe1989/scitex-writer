...
#          set clew_verify.level: off to silence.
#
...
#          strict precedence (CLI only ever tightens):
#            1. --strict CLI flag (presence => true; passes --strict to clew)
#            2. project ./config.yaml key clew_verify.strict
#            3. user ~/.scitex/writer/config.yaml clew_verify.strict
#            4. default false
#
# Usage:
...
def resolve_strict(cli_flag, project_dir):
    ...
    for cfg in (
        ...
        val = _read_block(cfg, "clew_verify").get("strict")
...
def main():
    ...
    clew_bin = _find_clew()
    if clew_bin is None:
        # Missing tool is an environment gap, not a verification failure -- warn
        # (even at error level) so a non-clew environment is never hard-blocked.
        log_warn(
    ...
    if code == 0:
        ...
        # NO_CLAIMS (20): nothing registered yet -- warn, never block.
        log_warn(label)
        log_detail(
