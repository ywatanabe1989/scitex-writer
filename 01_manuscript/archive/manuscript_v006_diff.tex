...
%DIF DEL ./01_manuscript/archive/manuscript_v005_diff.tex   Sat Sep 27 19:46:35 2025
%DIF ADD ./01_manuscript/manuscript.tex                     Sat Sep 27 20:37:06 2025
%DIF 2-22c2
%DIF < %DIF LATEXDIFF DIFFERENCE FILE
%DIF < %DIF DEL ./01_manuscript/archive/manuscript_v004_diff.tex   Sat Sep 27 19:12:03 2025
%DIF < %DIF ADD ./01_manuscript/manuscript.tex                     Sat Sep 27 19:46:20 2025
%DIF < %DIF 2-15c2
%DIF < %DIF < %DIF LATEXDIFF DIFFERENCE FILE
%DIF < %DIF < %DIF DEL ./01_manuscript/archive/manuscript_v003_diff.tex   Sat Sep 27 19:11:00 2025
%DIF < %DIF < %DIF ADD ./01_manuscript/manuscript.tex                     Sat Sep 27 19:11:47 2025
%DIF < %DIF < %DIF 2-9d2
%DIF < %DIF < %DIF < %DIF LATEXDIFF DIFFERENCE FILE
%DIF < %DIF < %DIF < %DIF DEL ./01_manuscript/archive/manuscript_v002_diff.tex   Sat Sep 27 19:09:36 2025
%DIF < %DIF < %DIF < %DIF ADD ./01_manuscript/manuscript.tex                     Sat Sep 27 19:10:44 2025
%DIF < %DIF < %DIF < %DIF 2-4d2
%DIF < %DIF < %DIF < %DIF < %DIF LATEXDIFF DIFFERENCE FILE
%DIF < %DIF < %DIF < %DIF < %DIF DEL ./01_manuscript/manuscript.tex   Sat Sep 27 19:09:21 2025
%DIF < %DIF < %DIF < %DIF < %DIF ADD ./01_manuscript/manuscript.tex   Sat Sep 27 19:09:21 2025
%DIF < %DIF < %DIF < %DIF -------
%DIF < %DIF < %DIF -------
%DIF < %DIF < %% Timestamp: "2025-09-27 14:50:40 (ywatanabe)"
%DIF < %DIF -------
...
\DIFaddbegin \begin{highlights}
\pdfbookmark[1]{Highlights}{highlights}
\DIFaddend 

%DIF < % \item Highlight \#1
\DIFaddbegin \item \DIFadd{Highlight \#1
}\DIFaddend 

%DIF < % \item Highlight \#2
\DIFaddbegin \item \DIFadd{Highlight \#2
}\DIFaddend 

%DIF < % \item Highlight \#3
\DIFaddbegin \item \DIFadd{Highlight \#3
}\DIFaddend 

%DIF < % \item Highlight \#4
\DIFaddbegin \item \DIFadd{Highlight \#4
}\DIFaddend 

%DIF < % \end{highlights}
\DIFaddbegin \end{highlights}
\DIFaddend 
...
\subsection{\DIFadd{Dataset Characteristics}}
\DIFadd{The NeuroVista dataset provided extensive long-term iEEG recordings from 9 patients with drug-resistant focal epilepsy. After excluding the initial 100-day post-implantation period, we analyzed a total of XX patient-years of continuous recording (mean: XX months per patient, range: 6-14 months). Across all patients, we identified and analyzed:
...
\DIFadd{The 16-channel electrode arrays provided comprehensive spatial coverage of the seizure onset zones, with an average of XX ± XX channels showing significant PAC modulation during seizure events.
}

\subsection{\DIFadd{Phase-Amplitude Coupling Patterns}}

\subsubsection{\DIFadd{Baseline PAC Characteristics}}
\DIFadd{During interictal periods (>4 hours from any seizure), we observed stable baseline PAC patterns with the following characteristics:
...
\subsubsection{\DIFadd{Pre-ictal PAC Evolution}}
\DIFadd{Analysis of the 60-minute pre-seizure period revealed progressive changes in PAC:
...
\subsubsection{\DIFadd{Ictal PAC Dynamics}}
\DIFadd{During seizures, PAC showed dramatic alterations:
...
\subsubsection{\DIFadd{Post-ictal Recovery}}
\DIFadd{Following seizure termination, PAC exhibited gradual normalization:
...
\subsection{\DIFadd{Spatial Distribution and Channel Consistency}}

\DIFadd{Analysis of PAC spatial patterns across the 16-channel arrays revealed:
...
\subsection{\DIFadd{Seizure Prediction Performance}}

\subsubsection{\DIFadd{Classification Accuracy}}
\DIFadd{Using PAC features extracted from 10-second windows, we trained patient-specific classifiers to discriminate pre-ictal from interictal states:
...
\subsubsection{\DIFadd{Pseudo-prospective Evaluation}}
\DIFadd{Testing on held-out data (post day 200) demonstrated:
...
\subsubsection{\DIFadd{Patient-specific Variability}}
\DIFadd{Performance varied across patients, correlating with:
- Seizure frequency (r = 0.XX, p < 0.05)
- Electrode coverage of onset zone (r = 0.XX, p < 0.05)
- Baseline PAC stability (r = -0.XX, p < 0.05)
}

\DIFadd{Best performing patients (n=3) achieved AUC > 0.XX with sensitivity > XX\% at <XX\% time in warning.
}

\subsection{\DIFadd{Temporal Patterns and Circadian Rhythms}}

\DIFadd{Long-term PAC analysis revealed multi-scale temporal patterns:
...
\subsection{\DIFadd{Comparison with Traditional Features}}

\DIFadd{PAC-based prediction outperformed conventional EEG features:
...
\DIFadd{Combined models incorporating PAC with traditional features achieved marginal improvement (AUC increase: 0.XX, p = 0.XX).
}

\label{sec:results}
\DIFaddend % Figure and table placeholders with descriptions
...
\section{Discussion}
\DIFdelbegin \DIFdel{Discussion here.}\DIFdelend \DIFaddbegin 

\subsection{\DIFadd{Principal Findings}}
\DIFadd{...
...
The processed PAC databases and analysis code are available at \DIFdelbegin %DIFDELCMD < \url{https://github.com/ywatanabe1989/neurovista-pac} %%%
...
%DIFDELCMD < %%%
\DIFdel{The SciTeX framework }\DIFdelend \DIFaddbegin \url{https://github.com/ywatanabe1989/neurovista}\DIFadd{. GPU-accerelated PAC calculation code is available as a standalone Python package `gpac` at }\url{https://github.com/ywatanabe1989/gPAC}\DIFadd{. The SciTeX Python utilities }\DIFaddend used for reproducible computing is available at \url{https://github.com/ywatanabe1989/SciTeX}.
...
%DIF > % Timestamp: "2025-09-27 20:17:53 (ywatanabe)"
%DIF > % File: "/ssh:sp:/home/ywatanabe/proj/neurovista/paper/01_manuscript/contents/additional_info.tex"
\DIFaddbegin 

\DIFaddend \pdfbookmark[1]{Additional Information}{additional_information}
...
This research was funded by \DIFdelbegin \DIFdel{...
}\DIFdelend \DIFaddbegin \hl{funding bodies here}
...
\DIFdelbegin %DIFDELCMD < \pdfbookmark[2]{Inclusion and Diversity Statement}{inclusion_and_diversity_statement}        
%DIFDELCMD < %%%
...
The authors employed \DIFdelbegin \DIFdel{Claude Code (provided by Anthropic }\DIFdelend \DIFaddbegin \DIFadd{large language models such as Claude (Anthropic Inc.}\DIFaddend ) for code development and \DIFdelbegin \DIFdel{ChatGPT (provided by OpenAI) for enhancing the }\DIFdelend \DIFaddbegin \DIFadd{complementing }\DIFaddend manuscript's English language quality. After incorporating \DIFdelbegin \DIFdel{the }\DIFdelend suggested improvements, the authors meticulously revised the content. Ultimate responsibility for the final content of this publication rests entirely with the authors.
...
\begin{figure*}[p]
    \DIFdelbeginFL %DIFDELCMD < \pdfbookmark[2]{Figure 00}{figure_id_00}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \pdfbookmark[2]{Figure 01}{figure_id_01}
    \DIFaddendFL \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\textwidth]{./01_manuscript/contents/figures/caption_and_media/jpg_for_compilation/Figure_ID_00_template.jpg}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\textwidth]{./01_manuscript/contents/figures/caption_and_media/jpg_for_compilation/Figure_ID_01_demographic_data_a.jpg}
    \DIFaddendFL \caption{
\DIFdelbeginFL \textbf{\DIFdelFL{Figure 00
}}
%DIFAUXCMD
...
\\
Description for figure \DIFdelbeginFL \DIFdelFL{00.
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{01.
}\DIFaddendFL }
    \DIFdelbeginFL %DIFDELCMD < \label{fig:00_template}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:01_demographic_data_a}
...
\begin{figure*}[p]
    ...
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\textwidth]{./01_manuscript/contents/figures/caption_and_media/jpg_for_compilation/Figure_ID_01_workflow.jpg}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\textwidth]{./01_manuscript/contents/figures/caption_and_media/jpg_for_compilation/Figure_ID_01_demographic_data_b.jpg}
    \DIFaddendFL \caption{
\textbf{
Figure 01
}
...
Description for figure 01.
    ...
    \DIFdelbeginFL %DIFDELCMD < \label{fig:01_workflow}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:01_demographic_data_b}
\end{figure*}
