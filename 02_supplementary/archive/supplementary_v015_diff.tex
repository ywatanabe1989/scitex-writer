...
%DIF DEL ./02_supplementary/archive/supplementary_v014.tex   Wed Nov 12 11:13:01 2025
%DIF ADD ./02_supplementary/supplementary.tex                Wed Nov 12 12:29:37 2025
% SciTeX Writer v2.0.0-rc2 (https://scitex.ai)
% LaTeX compilation engine: latexmk
%DIF 4c4
%DIF < % Compiled: 2025-11-12 11:12:49
%DIF -------
% Compiled: 2025-11-12 12:29:37 %DIF > 
%DIF -------
...
%% =============================================================================
%% Comparison: v014 → v014
%% Generated: 2025-11-12 12:29:48
...
\fancypagestyle{diffstyle}{
    ...
    \fancyhead[L]{\small\textit{Diff: v014 → v014}}
    \fancyhead[R]{\small\textit{supplementary}}
    \fancyfoot[L]{\small Generated: 2025-11-12 12:29}
...
\subsection*{\DIFdelbegin \DIFdel{Makefile }\DIFdelend \DIFaddbegin \DIFadd{Compilation }\DIFaddend Command Reference}

The \DIFdelbegin \DIFdel{Makefile provides a comprehensive set of targets for document compilation and management. The }\texttt{\DIFdel{make manuscript}} %DIFAUXCMD
\DIFdel{command compiles the main manuscript by first executing preprocessing scripts, then running pdflatex twice, followed by bibtex, and finally pdflatex twice more to resolve all cross-references. The }\texttt{\DIFdel{make clean}} %DIFAUXCMD
\DIFdel{target removes auxiliary files while preserving source content and compiled PDFs}\DIFdelend \DIFaddbegin \DIFadd{compilation scripts provide extensive command-line options for customizing the build process. Supplementary Table~\ref{tab:S1_compilation_options} documents all available options including engine selection, draft mode, component skipping, and performance tuning parameters. The system supports three compilation engines with distinct performance characteristics (Supplementary Table~\ref{tab:S2_compilation_engines}): Tectonic for ultra-fast incremental builds, latexmk for reliable smart recompilation, and traditional 3-pass for maximum compatibility.
}

\DIFadd{Configuration parameters are specified in YAML files located in the }\texttt{\DIFadd{config/}} \DIFadd{directory. Supplementary Table~\ref{tab:S3_yaml_configuration} details the available settings including citation style selection, engine preferences, and verbosity controls. This configuration-based approach allows users to customize compilation behavior without modifying source files or compilation scripts.
}

\DIFadd{The bibliography system supports over 20 citation styles (Supplementary Table~\ref{tab:S5_citation_styles}) covering major academic disciplines including sciences, engineering, social sciences, and humanities. Style switching requires only configuration file changes, with the system automatically applying appropriate formatting to all citations and bibliography entries}\DIFaddend . The \texttt{make archive} command creates a timestamped copy of the current manuscript in the archive directory using the format \texttt{manuscript\_vXXX.tex} where XXX is an automatically incremented version number. The \texttt{make diff} target executes latexdiff between the current version and the most recent archived version, producing a PDF with color-coded additions and deletions.

\subsection*{Preprocessing Pipeline Implementation}

\DIFdelbegin \DIFdel{Figure preprocessing involves scanning }\DIFdelend \DIFaddbegin \DIFadd{The preprocessing pipeline handles multiple asset types with format-specific processing. Supplementary Table~\ref{tab:S4_supported_formats} documents all supported file formats and auto-conversion capabilities. Figure preprocessing scans }\DIFaddend the \texttt{01\_manuscript/contents/figures/caption\_and\_media/} directory for \DIFdelbegin \DIFdel{subdirectories containing }\DIFdelend image files and corresponding \texttt{.tex} caption files. The script \DIFdelbegin \DIFdel{extracts the }\DIFdelend \DIFaddbegin \DIFadd{supports raster formats (PNG, JPEG, TIFF), vector graphics (SVG, PDF), and markup languages (Mermaid). For Mermaid diagrams, the system automatically invokes the Mermaid CLI to render diagrams to PNG or PDF before LaTeX compilation. The script extracts }\DIFaddend caption text, determines the appropriate image file based on priority ordering\DIFdelbegin \DIFdel{(PDF, then PNG, then JPEG)}\DIFdelend , and generates LaTeX figure inclusion code using the \texttt{graphicx} package.
\DIFdelbegin \DIFdel{The generated code maintains aspect ratios, sets maximum widths to column width, and includes proper labeling for cross-referencing}\DIFdelend \DIFaddbegin 

\DIFadd{Table preprocessing handles CSV files paired with caption definitions. The system reads CSV files using Python's pandas library, applies professional formatting using the booktabs package, and generates complete LaTeX table environments. Authors specify only the data (CSV) and caption (TEX), while the system handles all formatting details including column alignment, header styling, and row spacing}\DIFaddend . All generated figure \DIFaddbegin \DIFadd{and table }\DIFaddend code is concatenated into \DIFaddbegin \DIFadd{respective }\DIFaddend \texttt{FINAL.tex} \DIFdelbegin \DIFdel{which is }\DIFdelend \DIFaddbegin \DIFadd{files which are }\DIFaddend included by the main document. \DIFdelbegin \DIFdel{The table preprocessing follows an analogous workflow but handles the additional complexity of tabular environments and allows for both simple and complex multi-column layouts}\DIFdelend \DIFaddbegin \DIFadd{This separation of content from presentation enables authors to focus on data and scientific content rather than typesetting syntax}\DIFaddend .
...
%DIF >  Table from: 01_compilation_options.tex
\DIFaddbegin 
...
%DIFDELCMD < \pdfbookmark[1]{Tables}{tables}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{csv2latex translates a csv file to a LaTeX file
Example: csv2latex january_stats.csv > january_stats.tex
Usage: csv2latex }[\DIFadd{--nohead}] \DIFadd{(LaTeX) no document header: useful for inclusion
	}[\DIFadd{--longtable}] \DIFadd{(LaTeX) use package longtable: useful for long input
	}[\DIFadd{--noescape}] \DIFadd{(LaTeX) do not escape text: useful for mixed CSV/TeX input
	}[\DIFadd{--guess}] \DIFadd{(CSV) guess separator and block |
	}[\DIFadd{--separator <(c)omma|(s)emicolon|(t)ab|s(p)ace|co(l)on>}] \DIFadd{(CSV's comma)
	}[\DIFadd{--block <(q)uote|(d)ouble|(n)one>}] \DIFadd{(CSV) block delimiter (e.g: none)
	}[\DIFadd{--lines n}] \DIFadd{(LaTeX) rows per table: useful for long tabulars |
		}[\DIFadd{--font n}] \DIFadd{font size used (in pt)
	}[\DIFadd{--position <l|c|r>}] \DIFadd{(LaTeX) text align in cells
	}[\DIFadd{--colorrows graylevel}] \DIFadd{(LaTeX) alternate gray rows (e.g: 0.75)
	}[\DIFadd{--reduce level}] \DIFadd{(LaTeX) reduce table size (e.g: 1)
	}[\DIFadd{--landscape}] \DIFadd{(LaTeX) use landscape mode
	}[\DIFadd{--repeatheader}] \DIFadd{(LaTeX) repeat table header (for long tables)
	}[\DIFadd{--nohlines}] \DIFadd{(LaTeX) don't put hline between table rows
	}[\DIFadd{--novlines}] \DIFadd{(LaTeX) don't put vline between columns
		csv_file.csv
The "longtable" option needs the }{\DIFadd{longtable}} \DIFadd{LaTeX package
The "colorrows" option needs the }{\DIFadd{colortbl}} \DIFadd{LaTeX package
The "reduce" option needs the }{\DIFadd{relsize}} \DIFadd{LaTeX package
}\DIFaddend 
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\textbf{\DIFdelFL{Table 0: Placeholder}}%DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD <     \smallskip
%DIFDELCMD <     %%%
\DIFdelFL{To add tables to your manuscript:}%DIFDELCMD < \\
%DIFDELCMD <     %%%
\DIFdelFL{1. Place CSV files in }\texttt{\DIFdelFL{caption\_and\_media/}} %DIFAUXCMD
\DIFdelFL{with format }\texttt{\DIFdelFL{XX\_description.csv}}%DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD <     %%%
\DIFdelFL{2. Create matching caption files }\texttt{\DIFdelFL{XX\_description.tex}}%DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD <     %%%
\DIFdelFL{3. Reference in text using }\texttt{\DIFdelFL{Table\textasciitilde\textbackslash ref\{tab:XX\_description\}}}%DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD <     \smallskip
%DIFDELCMD <     %%%
\DIFdelFL{Example: }\texttt{\DIFdelFL{01\_seizure\_count.csv}} %DIFAUXCMD
...
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{Instructions}} %DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD <         \midrule
%DIFDELCMD <         %%%
\DIFdelFL{1. Add CSV}%DIFDELCMD < & %%%
\DIFdelFL{Place file like }\texttt{\DIFdelFL{01\_data.csv}} %DIFAUXCMD
\DIFdelFL{in }\texttt{\DIFdelFL{caption\_and\_media/}} %DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD <         %%%
\DIFdelFL{2. Add Caption }%DIFDELCMD < & %%%
\DIFdelFL{Create }\texttt{\DIFdelFL{01\_data.tex}} %DIFAUXCMD
\DIFdelFL{with table caption }%DIFDELCMD < \\
%DIFDELCMD <         %%%
\DIFdelFL{3. Compile }%DIFDELCMD < & %%%
\DIFdelFL{Run }\texttt{\DIFdelFL{./compile -m}} %DIFAUXCMD
\DIFdelFL{to process tables}%DIFDELCMD < \\
%DIFDELCMD <         %%%
\DIFdelFL{4. Reference }%DIFDELCMD < & %%%
\DIFdelFL{Use }\texttt{\DIFdelFL{\textbackslash ref\{tab:01\_data\}}} %DIFAUXCMD
\DIFdelFL{in manuscript }%DIFDELCMD < \\
...
%DIF >  ======================================================================
\DIFadd{csv2latex translates a csv file to a LaTeX file
Example: csv2latex january_stats.csv > january_stats.tex
Usage: csv2latex }[\DIFadd{--nohead}] \DIFadd{(LaTeX) no document header: useful for inclusion
	}[\DIFadd{--longtable}] \DIFadd{(LaTeX) use package longtable: useful for long input
	}[\DIFadd{--noescape}] \DIFadd{(LaTeX) do not escape text: useful for mixed CSV/TeX input
